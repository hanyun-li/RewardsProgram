...
// --------------------聊天室内页面----------------------------------------------------
	...
	$('.imgFileBtn').change(function(event) {
		...
		var str = '<img src="/chat/images/chatimg/1/201503/agafsdfeaef.jpg" />'
...
// -----下边的代码不用管---------------------------------------
	...
	$('.clapboard').click(function(event) {
		...
		$('.user_portrait img').attr('src', '/chat/images/user/' + $('.user_portrait img').attr('ptimg') + '.png');
	...
	$('.select_portrait img').hover(function() {
		...
		$('.user_portrait img').attr('src', '/chat/images/user/' + portrait_id + '.png');
	}, function() {
		var t_id = $('.user_portrait img').attr('portrait_id');
		$('.user_portrait img').attr('src', '/chat/images/user/' + t_id + '.png');
	});
	...
	function sends_message (userName, userPortrait, message) {
		if(message!='') {
			$('.main .chat_info').html($('.main .chat_info').html() + '<li class="right"><img src="/chat/images/user/' + userPortrait + '.png" alt=""><b>' + userName + '</b><i>09:15</i><div class="aaa">' + message  +'</div></li>');
		}
	}
